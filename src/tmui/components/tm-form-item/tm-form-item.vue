<template>
  <view :class="[`mx-${props.margin[0]}`]">
    <tm-sheet
      :transparent="props.transparent !== null ? props.transparent : tmFormtransparent"
      :round="props.round"
      :margin="[0, 0]"
    ...
    >
      <view :class="[`py-${props.margin[1]}`]">
        <view
          v-if="props.showError && props.showTopErrorGap"
          :style="{ height: `${props.errHeight}rpx` }"
        >
        </view>
        <view
          :class="[
            ...
            layout == 'horizontal' && !props.align ? 'flex-row-center-start' : '',
			<!-- #ifndef APP-NVUE -->
            layout == 'vertical' && !props.align ? 'flex-col' : '',
			<!-- #endif -->
            props.align,
            props.parentClass,
          ]"
        >
          <slot name="label">
            <view
              ...
              :style="[
                layout == 'horizontal'
                  ? { width: (props.labelWidth || tmFormLabelWidth) + 'rpx' }
                  : '',
              ]"
              class="flex flex-row"
              :class="[
                tmFormLabelAlign == 'right'
                  ? 'flex-row-center-end'
                  : 'flex-row-center-start',
                layout != 'horizontal' ? 'mb-24 flex-1 ' : 'mr-32 ',
              ]"
            >
              <view
                v-if="tmFormLabelAlign != 'right'"
                style="width: 12px"
              ...
              <view
                ...
                :class="[
                  ...
                  tmFormLabelAlign == 'right' && layout == 'horizontal'
                    ? 'text-align-right'
                    : '',
                  tmFormLabelAlign == 'center' && layout == 'horizontal'
                    ? 'text-align-center'
                    : '',
                ]"
                :style="[layout == 'horizontal' ? { width: '0px' } : '']"
              >
                <tm-text
                  :color="
                    tmFormFun == 'validate' &&
          ...
          <view class="flex-1" :style="[layout == 'horizontal' ? { width: '0px' } : '']">
            <slot></slot>
          </view>
        </view>
...
const tmFormtransparent = inject(
  "tmFormtransparent",
  computed(() => false)
);
