<template>
	<tm-sheet :eventPenetrationEnabled="true" :transparent="true" :margin="props.margin" :padding="props.padding">
		<tm-sheet
...
//是否聚焦中。
const _focusActive = ref(props.focus || false)
watch(
	() => props.focus,
	() => {
		_focusActive.value = props.focus
	}
)
...
function focus(e: any) {
	_focusActive.value = true
	emits('focus', e)
	// pushFormItem();
}
function blur(e: any) {
	_focusActive.value = false
	// pushFormItem();
	emits('blur', e)
}
function confirm() {
