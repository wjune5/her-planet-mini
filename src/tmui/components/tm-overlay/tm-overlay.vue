	...
	<!-- #endif -->
		...
		>
			<view
				ref="overlay"
				:class="[
					bgColor_rp && !props.transparent && ani ? 'blurOnOpacity' : 'blurOffOpacity', 
				'overlay',
				store.tmuiConfig?.themeConfig.overflowBlur&&bgColor_rp && !props.transparent && ani?'blurOn':'',
				store.tmuiConfig?.themeConfig.overflowBlur&&bgColor_rp && !props.transparent && !ani?'blurOff':'',
				]"
				:style="[
					bgColor_rp && !props.transparent ? { backgroundColor: showMask ? bgColor_rp : '' } : '',
...
const bgColor_rp = computed(() => {
  if (!props.bgColor || props.transparent) return "rgba(0,0,0,0)";
