<template>
  <tm-sheet :margin="[0, 0]" :padding="[0, 0]">
    <view v-if="props.quickBtnShow && props.quickBtn.length > 0">
    ...
    <view>
      ...
      <!-- <tm-time-picker
        ...
        :end="endTtime"
      >
        <tm-cell title="时分" :right-text="picker_focus_date_str || '请选择'"></tm-cell>
      </tm-time-picker> -->
    </view>
...
import { validateIdCard } from "../tm-keyboard/card";
...
const props = defineProps({
   ...
   * 这里设置的意义是：当你需要同步日期数据，由用户设置时，如果你设置了一个确认按钮来同步用户设置的日期，那么这里可以设置为false，
...
function changeFocus(index: number) {
  ...
  if (index == 0 && !valuedate_start.value) {
    vstd = DayJs(valuedate_end.value).isValid()
      ? valuedate_end.value
      : DayJs().format(defaultFormat);
  }
  if (index == 1 && !valuedate_end.value) {
    vsed = DayJs(valuedate_start.value).isValid()
