...
import { debugWarn } from '@/tn/utils/error'

import type { ComponentInternalInstance } from 'vue'

export const useScrollTransparentNavbar = (
  ...
  // 定义滚动到对应位置的元素的id
  const triggerElementId = `tse-${uni.$tm.u.getUid(1,6)}`
  ...
  const _getTriggerElementRectInfo = async () => {
    try {
      const res = await uni.$tm.u.queryDom(instance, `#${triggerElementId}`)
      if (!res.top) {
